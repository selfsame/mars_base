...
(function() {
  ...
  Engineer = (function(_super) {
    ...
    Engineer.prototype.idle = function() {
      var debree, thing, tile, _i, _len;
      if (this.remove_order) {
        this.remove_order.destroy();
        return this.remove_order = false;
      } else if (this.build_order) {
        if (this.tile_pos[0] === this.build_order.x && this.tile_pos[1] === this.build_order.y) {
        ...
        if ((window.Floors.under_construction != null) && window.Floors.under_construction.length > 0) {
          ...
          if (this.path_to([tile.x, tile.y])) {
            ...
            if (debree) {
              for (_i = 0, _len = debree.length; _i < _len; _i++) {
                thing = debree[_i];
                if (thing.tile_pos[0] === tile.x && thing.tile_pos[1] === tile.y) {
                  console.log('removing ', thing);
                  this.remove_order = thing;
                  this.state = 'removing_object';
                  return;
                }
              }
            }
